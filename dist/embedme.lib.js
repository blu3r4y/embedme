...
/* @internal */
function getReplacement(inputFilePath, options, logMethod, substr, leadingSpaces, lineEnding, infoString, codeExtension, firstLine, startLineNumber, ignoreNext, commentEmbedOverrideFilepath) {
    /**
...
function embedme(sourceText, inputFilePath, options) {
    ...
    while ((result = codeFenceFinder.exec(sourceText)) !== null) {
        ...
        const replacement = getReplacement(inputFilePath, options, log, codeFence, leadingSpaces, lineEnding, infoString, codeExtension, firstLine || '', startLineNumber, /<!--\s*?embedme[ -]ignore-next\s*?-->/g.test(start), commentInsertion ? commentInsertion[1] : undefined);
